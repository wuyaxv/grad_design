...
def accept(port):
    ...
    s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)         # Allow reuse of the adda.
    s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEPORT, 1)         # Allow reuse of the port.
    s.bind(('', port))                                              # bind to the local port
    s.listen(1)
    s.settimeout(5)
...
def main(host='54.187.46.146', port=5005):
    ...
    priv_addr = sa.getsockname()                # 获得内网的ip地址和端口信息
